...
This test suite verifies the correctness of `stitch_checkpoint.py` by
...
class TestOmniCheckpointStitcher(unittest.TestCase):
  ...
  def _restore_checkpoints(self, config):
    ...
    with jax.set_mesh(self.test_mesh):
      ...
      llm_restored = stitch_checkpoint._restore_subtrees_from_path(self.llm_ckpt_dir, llm_abstract, ckptr)
    return (
        stitched_inner,
        vision_restored,
        llm_restored,
