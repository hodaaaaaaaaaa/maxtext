...
class Decoder(nn.Module):
  ...
  def _apply_deepseek_scanned_blocks(
    ...
    assert (
        len(rematted_block_layers) == 2
    ), "Scanned layers must have a length of 2 using deepseek."
    layer_call_kwargs = {
        "previous_chunk": previous_chunk,
        "slot": slot,
    }
    dense_layer = rematted_block_layers[0]
    moe_layer = rematted_block_layers[1]
    if cfg.engram_layers:
      original_dense_call = dense_layer.__call__
      original_moe_call = moe_layer.__call__
      dense_layer.__call__ = functools.partial(
          dense_layer.__call__, **layer_call_kwargs
      )
      moe_layer.__call__ = functools.partial(
          moe_layer.__call__, **layer_call_kwargs
      )
      ...
      num_moe_layers = cfg.num_decoder_layers - cfg.first_num_dense_layers
      if getattr(cfg, "use_lineage", False) and not self.is_mutable_collection(
          "params"
      ):
        ...
        if cfg.use_batch_split_schedule and not self.is_mutable_collection(
            "params"
        ):
