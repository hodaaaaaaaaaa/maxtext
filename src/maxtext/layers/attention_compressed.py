...
class DeepseekV4Indexer(nnx.Module):
  ...
  def __init__(
    ...
    self.weights_proj = DenseGeneral(
        ...
        axis=-1,
        kernel_init=kernel_init,
        kernel_axes=("embed_attn", "indexer_weights"),
        dtype=self.dtype,
        weight_dtype=self.weight_dtype,
        quant=quant,
        matmul_precision=config.matmul_precision,
...
class DeepseekV4CSACompressor(BaseDeepseekCompressor):
  ...
  def __call__(
  ...
  ) -> Tuple[Array, Array] | Tuple[Array, Array, Optional[Array]]:
    ...
    if compressed_len == 0:
      ...
      if return_indexer_scores:
        return compressed_kv, empty_mask, indexer_scores
      return compressed_kv, empty_mask
    ...
    if return_indexer_scores:
      return compressed_kv, compressed_mask, indexer_scores
...
class CompressedAttention(Attention):
  ...
  def _init_projections(self, inputs_q_shape: Tuple, inputs_kv_shape: Tuple) -> None:
    ...
    self.wkv = DenseGeneral(
        ...
        axis=-1,
        kernel_init=self.kernel_init,
        kernel_axes=("embed_attn", "kv_heads", "kv_head_dim"),
        dtype=self.dtype,
        weight_dtype=self.weight_dtype,
        quant=self.quant,
        matmul_precision=self.config.matmul_precision,
    ...
    self.o_b_proj = DenseGeneral(
        ...
        axis=-1,
        kernel_init=self.kernel_init,
        kernel_axes=("o_lora_up_proj", "embed_attn"),
        dtype=self.dtype,
        weight_dtype=self.weight_dtype,
        quant=self.quant,
        matmul_precision=self.config.matmul_precision,
